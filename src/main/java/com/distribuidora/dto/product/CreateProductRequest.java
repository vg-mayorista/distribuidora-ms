...
public record CreateProductRequest(
        ...
        @Schema(description = "Nombre único del producto", example = "Yerba Mate 1kg", requiredMode = Schema.RequiredMode.REQUIRED)
        @NotBlank
        @Size(max = 100)
        String name,
        ...
        @Schema(description = "Precio unitario por pack/caja (debe ser >= 0.00)", example = "5500.00", requiredMode = Schema.RequiredMode.REQUIRED)
        @NotNull
        @DecimalMin(value = "0.00", inclusive = true)
        @Digits(integer = 7, fraction = 2)
        BigDecimal price,

        @Schema(description = "Stock disponible en unidades físicas", example = "120", requiredMode = Schema.RequiredMode.NOT_REQUIRED)
        @Min(0)
        Integer stock,
